...
typedef struct
{
    ...
    // SPI接口
    SPIInstance *spi_gyro; // 注意,SPIInstnace内部也有一个GPIO_Instance_t,用于控制片选CS
    SPIInstance *spi_acc;  // 注意,SPIInstnace内部也有一个GPIO_Instance_t,用于控制片选CS
...
typedef struct
{
    ...
    GPIO_Init_Instance_t gyro_int_config;
    GPIO_Init_Instance_t acc_int_config;
    PID_Init_Config_s heat_pid_config;
    PWM_Init_Instance_t heat_pwm_config;
...
void BMI088AccSPIFinishCallback(SPIInstance *spi);
void BMI088GyroSPIFinishCallback(SPIInstance *spi);
void BMI088AccINTCallback(GPIO_Instance_t *gpio);
void BMI088GyroINTCallback(GPIO_Instance_t *gpio);
#endif // !__BMI088_H__