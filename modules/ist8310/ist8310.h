...
typedef struct tempist8310
{
    ...
    GPIO_Instance_t *gpio_rst;  // gpio实例,用于复位
    GPIO_Instance_t *gpio_exti; // gpio实例,用于获取MAG_DRDY引脚状态,判断数据是否准备好(EXTI外部中断)
...
typedef struct
{
    ...
    GPIO_Init_Instance_t gpio_conf_rst;  // gpio初始化配置,用于复位ist8310,看数据手册
    GPIO_Init_Instance_t gpio_conf_exti; // gpio初始化配置,用于获取MAG_DRDY引脚状态,判断数据是否准备好(EXTI外部中断)
